...
async function ensureSchema() {
  await env.DB.batch([
    env.DB.prepare("CREATE TABLE IF NOT EXISTS profiles (email TEXT PRIMARY KEY NOT NULL, name TEXT NOT NULL, birth_date TEXT NOT NULL, city TEXT NOT NULL, level TEXT NOT NULL, bio TEXT NOT NULL, created_at INTEGER NOT NULL, updated_at INTEGER NOT NULL)"),
...
export async function POST(request: Request) {
  ...
  const fields = Object.fromEntries(["name", "birthDate", "city", "level", "bio"].map(key => [key, String(form.get(key) ?? "").trim()]));
  if (Object.values(fields).some(value => !value)) return Response.json({ error: "Preencha todos os campos." }, { status: 400 });
  const now = Date.now();
  await env.DB.prepare("INSERT INTO profiles (email,name,birth_date,city,level,bio,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?) ON CONFLICT(email) DO UPDATE SET name=excluded.name,birth_date=excluded.birth_date,city=excluded.city,level=excluded.level,bio=excluded.bio,updated_at=excluded.updated_at")
    .bind(user.email, fields.name, fields.birthDate, fields.city, fields.level, fields.bio, now, now).run();
