...
type Draft = { name: string; birthDate: string; city: string; level: string; bio: string };
...
export default function CorrenteApp({ user }: { user: Viewer }) {
  ...
  const [draft, setDraft] = useState<Draft>({ name: user?.name ?? "", birthDate: "", city: "", level: "Intermediário", bio: "" });
  ...
  return (
    <main>
      ...
      <section className="content">
        ...
        {tab === "descobrir" && <>
          ...
          <div className={`card ${notice ? "leaving" : ""}`}>
            ...
            <div className="info"><div className="name"><h1>{profile.name}, {profile.age}</h1><span>●</span></div><p className="place">⌖ {profile.spot} · {profile.distance}</p><div className="surfline"><b>{profile.level}</b><span>•</span>{profile.wave}</div><p className="bio">{profile.bio}</p><div className="tags">{profile.tags.map(tag => <span key={tag}>{tag}</span>)}</div></div>
      ...
      <nav className="bottomnav" aria-label="Navegação principal"><button className={tab === "descobrir" ? "active" : ""} onClick={() => setTab("descobrir")}><span>⌁</span>Descobrir</button><button className={tab === "maré" ? "active" : ""} onClick={() => setTab("maré")}><span>♥</span>Sua maré<i>2</i></button><button className={tab === "mensagens" ? "active" : ""} onClick={() => setTab("mensagens")}><span>▱</span>Mensagens<i>1</i></button></nav>

      {signup && <div className="modalback" role="presentation"><section className="signup" role="dialog" aria-modal="true" aria-labelledby="signup-title"><button className="close" onClick={() => setSignup(false)} aria-label="Fechar">×</button><p className="eyebrow">ENTRE PARA A CORRENTE</p><h2 id="signup-title">Crie seu perfil</h2><p className="intro">Mostre quem você é dentro e fora d’água.</p>{!user && <a className="signin" href="/signin-with-chatgpt?return_to=%2F">Entrar com ChatGPT</a>}{user && <form onSubmit={submit}><label>Nome<input required value={draft.name} onChange={e => setDraft({...draft,name:e.target.value})}/></label><div className="twocol"><label>Nascimento<input required type="date" value={draft.birthDate} onChange={e => setDraft({...draft,birthDate:e.target.value})}/></label><label>Nível<select value={draft.level} onChange={e => setDraft({...draft,level:e.target.value})}><option>Iniciante</option><option>Intermediário</option><option>Avançado</option><option>Profissional</option></select></label></div><label>Cidade / praia base<input required placeholder="Ex.: Ubatuba, SP" value={draft.city} onChange={e => setDraft({...draft,city:e.target.value})}/></label><label>Sobre você<textarea required maxLength={280} placeholder="Seu estilo de vida, onda favorita e o que procura…" value={draft.bio} onChange={e => setDraft({...draft,bio:e.target.value})}/></label><div className="photolabel"><b>Suas fotos</b><span>{photos.length}/3</span></div><div className="photoGrid">{[0,1,2].map(i => <div className="photoSlot" key={i}>{previews[i] ? <img src={previews[i]} alt={`Foto ${i+1} selecionada`}/> : <span>{i === 0 ? "+" : "○"}</span>}</div>)}</div><label className="upload">Escolher até 3 fotos<input type="file" accept="image/jpeg,image/png,image/webp" multiple onChange={pickPhotos}/></label><small>JPG, PNG ou WebP. Máximo de 5 MB por foto.</small><button className="save" disabled={saving || photos.length === 0}>{saving ? "Salvando…" : saved ? "Perfil criado ✓" : "Criar meu perfil"}</button></form>}</section></div>}
    </main>
  );
}
